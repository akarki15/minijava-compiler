...
public class Phase1
{
	private List <Var> varList; //construct List <Var> of parameters
    ...
    List<Var> process(AListParamlist n) { 
        ...
        //n.getId();				// yields TId        
    	varList=new ArrayList <Var>(); // Reset the varList to add parameters for a new method
        varList.add(new Var(n.getId().toString().trim(), process(n.getType()))); //adds the first parameter into varList
                
	for (PParam p : n.getParam()){
	    varList.add(process(p));				// process(PParam)		    
	}
	return varList;
        //throw new UnsupportedOperationException ();     // remove when method is complete
    }
    ...
    Var process(AParam n) {    	
    	if (n.getType().equals("void ")){
    		...
    		return new Var(n.getId().toString().trim(), process(n.getType()));		    		
    	}       
    }
